...
/**
 * Extractor de las ligas de la FEB desde baloncestoenvivo.feb.es: lo usan
 * `feb.ts` (Primera FEB) y `feb2.ts` (el grupo de Segunda FEB del que sube un
 * equipo a la Primera del juego). `--force` lo vuelve a descargar todo.
 *
...
/** Una liga de la FEB: qué competición de la web y cómo se llama en el juego. */
export interface FebLeagueOptions {
  /** El `nm` y el `g` de las URL de la web (`primerafeb`, 1). */
  nm: string;
  group: number;
  /** La fase de liga regular en el desplegable de la página de resultados. */
  regularPhase: RegExp;
  /** Clave de caché de la clasificación, que sale de dos postbacks seguidos. */
  standingsCacheKey: string;
  competitionId: string;
  name: string;
  shortName: string;
  /** Nombre del fichero: `.real-data-cache/sources/<slug>-<año>.json`. */
  slug: string;
  /**
   * Si de la liga sólo entran algunos equipos, en otra liga del juego. Sólo a
   * ésos se les busca entrenador: los demás sólo sirven de escala.
   */
  guest?: SourceGuest;
}

function calendarUrl(options: FebLeagueOptions): string {
  return `${BASE}/calendario.aspx?g=${options.group}&t=${SEASON_START_YEAR}&nm=${options.nm}`;
}

function resultsUrl(options: FebLeagueOptions): string {
  return `${BASE}/resultados.aspx?g=${options.group}&t=${SEASON_START_YEAR}&nm=${options.nm}`;
}
 ...
 */
async function regularSeasonStandings(
  client: HttpClient,
  options: FebLeagueOptions
): Promise<string> {
  const RESULTS_URL = resultsUrl(options);
  const cacheKey = options.standingsCacheKey;
  ...
  const group = selectOptions(page, 'gruposDropDownList').find((option) =>
    options.regularPhase.test(option.label)
  );
...
export async function extractFebLeague(options: FebLeagueOptions): Promise<void> {
  const { force } = cliOptions();
  const client = createHttpClient({ minDelayMs: 2_000, force, log });
  const started = Date.now();

  log(`${options.name}: calendario y clasificación…`);
  const calendarTeams = parseCalendarTeams(await client.get(calendarUrl(options)));
  const standings = parseStandings(await regularSeasonStandings(client, options));
  ...
  for (const standing of standings) {
    ...
    for (const playerId of ids) {
      ...
      let page: FebPlayerPage | undefined;
      if (!roster || !stats) {
        try {
          page = parsePlayerPage(
            await client.get(`${BASE}/Jugador.aspx?i=${standing.teamId}&c=${playerId}`)
          );
        } catch (error) {
          // Alguna ficha ya no existe (404): se sigue con lo que haya.
          warn(`${name}: sin ficha del jugador ${playerId} (${(error as Error).message})`);
        }
      }
      players.push(playerFrom(playerId, roster, stats, page, name));
    }

    const enters = !options.guest || options.guest.teamIds.includes(standing.teamId);
    teams.push({
      ...
      players,
      coach: enters ? coachFrom(teamPage.coach, standing.teamId, name) : null
    });
    if (enters && !teams.at(-1)?.city)
      warn(`${name}: no se ha podido sacar la ciudad de la dirección`);
  }
  ...
  const file = sourceFile(options.slug, SEASON_START_YEAR);
